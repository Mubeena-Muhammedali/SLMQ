...
class AccountMove(models.Model):
	...
	def _get_default_journal(self):
		...
		elif self._context.get('default_od_is_bank_voucher', False):
	...
	def _compute_suitable_journal_ids(self):
		for m in self:
			...
			elif m.invoice_filter_type_domain == 'bank_voucher':
	...
	def _compute_invoice_filter_type_domain(self):
		for move in self:
			...
			elif move.od_is_bank_voucher:
				move.invoice_filter_type_domain = 'bank_voucher'
			else:
		...
		for record in self:
			if record.asset_category_id:
				vals = {
					...
					'od_cost':record.price_subtotal,
					'od_purchase_date' : record.move_id.invoice_date,
				}
