...
class ReportAgedPartnerBalance(models.AbstractModel):
    _name = 'report.orchid_account_enhancement_v14.report_agedpartnerbalance'
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move,
        ...
        # +120     : 2018-10-10
        period_length = data_form.get('period_length')
        periods = {}
        ...
        where_qry=""
        if data_form.get('partner_ids'):
            where_qry+=""" AND l.partner_id IN %s """
            arg_list += (tuple(data_form.get('partner_ids')),)
        if data_form.get('user_ids'):
            where_qry+=""" AND am.invoice_user_id IN %s """
            arg_list += (tuple(data_form.get('user_ids')),)
        # query = '''
        ...
        query = '''
            ...
            WHERE (l.account_id = account_account.id)
                ...
                AND ''' + reconciliation_clause + '''
                AND (l.date <= %s)
                AND l.company_id IN %s '''+ where_qry + '''
            ORDER BY UPPER(res_partner.name)'''
        ...
        if data_form.get('partner_ids'):
            partners = [sub for sub in partners if ((sub['partner_id'] in data_form.get('partner_ids')) or sub['partner_id'] is None)]
        ...
        for line in self.env['account.move.line'].browse(aml_ids):
            ...
            if data_form.get('partner_ids'):
                if not (line.partner_id.id in data_form.get('partner_ids')):
                    continue
            if data_form.get('user_ids'):
                if not (line.move_id.invoice_user_id.id in data_form.get('user_ids')):
                    continue
