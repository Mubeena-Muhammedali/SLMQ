	...
	def check_lead_probability(self):
		for record in self:
			if record.active:
				...
				# p_stage_id = c_stage_id
				n_stage_id = c_stage_id
				if c_stage_id.id!=1:
					# p_id = record.stage_id.id-1
					n_id = record.stage_id.sequence+1

					
					# p_stage_id = record.env['crm.stage'].browse(p_id)
					n_stage_id = record.env['crm.stage'].search([('sequence','=',n_id)])
				if c_stage_id.id != n_stage_id.id:
				# if c_stage_id.id != p_stage_id.id and not (c_stage_id.id==5):
					c_stage_probability = c_stage_id.probability
					# p_stage_probability = p_stage_id.probability
					n_stage_probability = n_stage_id.probability
					lead_probability = record.probability

					# if not (p_stage_probability <lead_probability <=c_stage_probability):
					if not (n_stage_probability >lead_probability >=c_stage_probability) and c_stage_probability!=100 and c_stage_id.id!=6 :
						# raise UserError(_("The probability should be between %s and %s.")%(p_stage_probability,c_stage_probability))##Upper limit as base commented
