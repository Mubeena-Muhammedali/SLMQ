...
class ReportFinancial(models.AbstractModel):
    _name = 'report.orchid_account_enhancement_v14.report_cash_flow'
    _description = 'Cash Flow Report'

    def _compute_account_balance(self, accounts):
        print("accountddddddd",accounts)
        mapping = {
        ...
        if accounts:
            ...
            print("jjjjjjoooooo",request,params)
    ...
    #         print("jjkkkkkkkkk",report.type,report.account_ids,report.name,report.parent_id)
    #         if report.type == 'accounts':
    #             print("type okkkkk",report.name,report.parent_id.name)
    #             # it's the sum of credit or debit
    #             res2 = self._compute_report_balance(report.parent_id)
    #             print("resssss2222",res2)
    #             for key, value in res2.items():
    #                 print("keyvallll",key,value)
    #                 cash_in_operation = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_in_from_operation0')
    #                 cash_out_operation = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_out_operation1')
    #                 cash_in_financial = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_in_financial0')
    #                 cash_out_financial = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_out_financial1')
    #                 cash_in_investing = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_in_investing0')
    #                 cash_out_investing = self.env.ref(
    #                     'orchid_account_enhancement_v14.cash_out_investing1')
    ...
    #             res[report.id]['account'] = self._compute_account_balance(
    #                 report.account_ids)
    #             print("resssssummm",res[report.id]['account'],self._compute_account_balance(
    #                 report.account_ids))
    ...
    def get_account_lines(self, data):
        print("functionnnnnnnnnnnnnnnnnnnnnnnn")
        print("functionnnnnnnnnnnnnnnnnnnnnnnn")
        lines = []
        account_report = self.env['account.financial.report'].search(
            [('id', '=', data['account_report_id'][0])])
        child_reports = account_report._get_children_by_order()
        print("kliiiiichuilkdddddd",child_reports)
    ...
    def _get_report_values(self, docids, data=None):
        ...
        print("kkkkkkkkkkk",report_lines)
        return {
            'doc_ids': self.ids,
            'doc_model': model,
            'data': data['form'],
