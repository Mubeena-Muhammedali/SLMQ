...
class MultipleInvoiceLayout(models.TransientModel):
        ...
        for wizard in self:
            if wizard.company_id:
                ...
                wizard.preview = ir_ui_view._render_template(
                    'orchid_account_enhancement_v14.multiple_invoice_wizard_preview',
                    {'company': wizard.company_id, 'preview_css': preview_css,
                     ...
                     'mi': self.env.ref(
                         'orchid_account_enhancement_v14.multiple_invoice_sample_name')
                     })
            else:
    ...
    def _get_layout_for_preview(self):
        if self.layout == 'web.external_layout_boxed':
            new_layout = 'orchid_account_enhancement_v14.boxed'

        elif self.layout == 'web.external_layout_clean':
            new_layout = 'orchid_account_enhancement_v14.clean'

        elif self.layout == 'web.external_layout_background':
            new_layout = 'orchid_account_enhancement_v14.background'

        else:
            new_layout = 'orchid_account_enhancement_v14.standard'
