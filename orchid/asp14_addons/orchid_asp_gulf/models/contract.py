	...
	def button_expire(self):
		...
		for record in records: 
			if not record.contract_line_ids.filtered(lambda r: r.state in('0_draft','active')):
				...
				print("'hereeeeeeee",record,record.date_to)
	...
	def button_renew(self, new_quotation):
		for record in self:
			...
			if new_quotation:
				...
				record.renewed=True
				record.contracts_to_renew = False
				print("quotatttt",renewed_quotation_id)
				# return {
				#   'view_type': 'form',
				#   "view_mode": 'form',
				#   'res_model': 'sale.order',
				#   'res_id': renewed_quotation_id.id,
...
class OrchidASPContractLines(models.Model):
	...
	def button_expire(self):
		...
		# need to check 
		print("hhhhhhhhhhhhhjjjjjjjjjjjj")
		records = self.search([
			('state', 'in', ['0_draft','active']),
			('billing_to', '<', fields.Date.today(self)),
		])
		print("recorddd",records)
		for record in records:
