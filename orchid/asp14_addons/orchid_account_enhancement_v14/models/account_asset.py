...
class AccountAssetAsset(models.Model):
    ...
    od_amount_per_day = fields.Float('Amount Per Day')
    ...
    def compute_depreciation_board(self):
        ...
        total_amount = 0
        amount_check = self.value - self.salvage_value
        ...
        if self.value_residual != 0.0:
            ...
            if self.prorata and self.od_prorata_days:
                dep_date = datetime.strptime(str(self._get_last_depreciation_date()[self.id]), DF).date()
                depreciation_date = self.last_day_of_month(dep_date)
                delta = depreciation_date - dep_date
                no_of_days = delta.days + 1
#                 last_date = depreciation_date + relativedelta(months=self.method_number)
                new_ldate = dep_date + relativedelta(months=self.method_number,days=-1)
                total_delta = new_ldate - dep_date
                total_days = total_delta.days +1
                if not total_days:
                    amount_per_day = 0
                else:
                    amount_per_day = self.value_residual/total_days
#                self.od_amount_per_day = amount_per_day
                for x in range(len(posted_depreciation_line_ids), undone_dotation_number):
                    ...
                    next_date = (datetime(year, month, 1) + relativedelta(months=+self.method_period))
        ...
        if category_id:
            ...
            return {
                'value': {
                    ...
                    'od_prorata_days': category.od_prorata_days,
                }
            }
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            # orchid
            od_cost_center_id = line.asset_id.od_cost_center_id and line.asset_id.od_cost_center_id.id or False
            move_line_1 = {
                'name': asset_name,
                ...
                'orchid_cc_id':od_cost_center_id,
            }
            move_line_2 = {
                'name': asset_name,
